    ...
    names.forEach(function (_name) {
      ...
      if (name.namespace !== 'base' && name.value === '') {
        ...
        for (const namespace in _this.callbacks) {
          if (
            _this.callbacks[namespace] instanceof Object && _this.callbacks[namespace][name.value] instanceof Array
          ) {
            delete _this.callbacks[namespace][name.value]
            // Remove namespace if empty
            if (
              Object.keys(_this.callbacks[namespace])
                .length === 0
            ) {
        ...
        _this.callbacks[name.namespace] instanceof Object && _this.callbacks[name.namespace][name.value] instanceof Array
      ) {
        delete _this.callbacks[name.namespace][name.value]
        // Remove namespace if empty
        if (
          Object.keys(_this.callbacks[name.namespace]).length === 0
        ) {
          delete _this.callbacks[name.namespace]
        }
      }
    ...
    if (name.namespace === 'base') {
      ...
      for (const namespace in _this.callbacks) {
        if (
          ...
          _this.callbacks[namespace][name.value].forEach(function (
            callback
          ) {
      ...
      _this.callbacks[name.namespace][name.value].forEach(function (
        callback
      ) {
        result = callback.apply(_this, args)
        if (typeof finalResult === 'undefined') {
